...
{
	EState *estate;
	HashState *outerHashNode; // Symmetric hash join project
	HashState *innerHashNode; // Symmetric hash join project
	List *joinqual;
	List *otherqual;

	TupleTableSlot *inntuple;
	TupleTableSlot *outtuple; // Symmetric hash join project
	ExprContext *econtext;
	ExprDoneCond isDone;
	HashJoinTable innerHashtable; // Symmetric hash join project
	HashJoinTable outerHashtable; // Symmetric hash join project

	HeapTuple curtuple;
	TupleTableSlot *innerTupleSlot; // Symmetric hash join project
	...
	innerHashNode = (HashState *)innerPlanState(node); // Symmetric hash join project
	outerHashNode = (HashState *)outerPlanState(node); // Symmetric hash join project

	/*
	 * get information from HashJoin state
	 */
	innerHashtable = node->hj_InnerHashTable; // Symmetric hash join project
	outerHashtable = node->hj_OuterHashTable; // Symmetric hash join project
	 ...
	 */
	if (innerHashtable == NULL && outerHashtable == NULL) // Symmetric hash join project
	{
		/*
		...
		outerHashtable = ExecHashTableCreate((Hash *)outerHashNode->ps.plan, node->hj_HashOperators); // Symmetric hash join project
		node->hj_OuterHashTable = outerHashtable;													  // Symmetric hash join project

		/*
		 * execute the Hash node, to build the hash table
		 */
		innerHashNode->hashtable = innerHashtable; // Symmetric hash join project
		outerHashNode->hashtable = outerHashtable; // Symmetric hash join project
	}

	/*
	 * Symmetric hash join project: run the hash join process
	 */
	for (;;)
	{
...
ExecInitHashJoin(HashJoin *node, EState *estate)
{
	...
	Hash *outerHashNode; // Symmetric hash join project
	Hash *innerHashNode; // Symmetric hash join project
	List *lclauses;
	List *rclauses;
	List *hoperators;
	ListCell *l;
	 ...
	 */
	outerHashNode = (Hash *)outerPlan(node); // Symmetric hash join project
	innerHashNode = (Hash *)innerPlan(node); // Symmetric hash join project

	outerPlanState(hjstate) = ExecInitNode((Plan *)outerHashNode, estate); // Symmetric hash join project
	innerPlanState(hjstate) = ExecInitNode((Plan *)innerHashNode, estate); // Symmetric hash join project
...
#define HASHJOIN_NSLOTS 3
	...
	hjstate->hj_InnerTupleSlot = ExecInitExtraTupleSlot(estate); // Symmetric hash join project
...

	{
		...
		hashstate = (HashState *)outerPlanState(hjstate); // Symmetric hash join project
		slot = hashstate->ps.ps_ResultTupleSlot;		  // Symmetric hash join project
		hjstate->hj_OuterHashTupleSlot = slot;			  // Symmetric hash join project
	}
	...
	ExecSetSlotDescriptor(hjstate->hj_InnerTupleSlot,
						  ExecGetResultType(innerPlanState(hjstate)),
						  false); // Symmetric hash join project
	...
	hjstate->hj_OuterHashTable = NULL;		// Symmetric hash join project
	hjstate->hj_FirstInnerTupleSlot = NULL; // Symmetric hash join project

	hjstate->hj_InnerCurHashValue = 0;
	hjstate->hj_InnerCurBucketNo = 0;
	hjstate->hj_InnerCurTuple = NULL;

	hjstate->hj_OuterCurHashValue = 0; // Symmetric hash join project
	hjstate->hj_OuterCurBucketNo = 0;  // Symmetric hash join project
	hjstate->hj_OuterCurTuple = NULL;  // Symmetric hash join project
	...
	((HashState *)outerPlanState(hjstate))->hashkeys = lclauses; // Symmetric hash join project

	hjstate->js.ps.ps_OuterTupleSlot = NULL;
	hjstate->js.ps.ps_InnerTupleSlot = NULL; // Symmetric hash join project
	hjstate->js.ps.ps_TupFromTlist = false;
	hjstate->hj_NeedNewOuter = true;
	hjstate->hj_NeedNewInner = true; // Symmetric hash join project
	hjstate->hj_MatchedOuter = false;

	// Symmetric hash join project
...
void ExecEndHashJoin(HashJoinState *node)
{
	...
	if (node->hj_OuterHashTable) // Symmetric hash join project
	{
		ExecHashTableDestroy(node->hj_OuterHashTable); // Symmetric hash join project
		node->hj_OuterHashTable = NULL;				   // Symmetric hash join project
	}
	...
	ExecClearTuple(node->hj_InnerTupleSlot); // Symmetric hash join project
	ExecClearTuple(node->hj_InnerHashTupleSlot);
	ExecClearTuple(node->hj_OuterHashTupleSlot); // Symmetric hash join project
 ...
 * will get messed up.
 */
void ExecHashJoinSaveTuple(HeapTuple heapTuple, uint32 hashvalue,
						   BufFile **fileptr)
{
